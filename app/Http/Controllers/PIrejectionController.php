...
class PIrejectionController extends Controller
{
   ...
   public function getRejection(Request $request)
   {
      ...
      if ($filter == 'monthly') {
         ...
         $chart1cDataShift = $this->getChart1cDataShift($tahun, $bulan, $tanggal);
         $getRejectionPerTypePartData = $this->getRejectionPerTypePartData($tahun, $bulan);
         $getRejectionTypeByShift = $this->getRejectionTypeByShift($tahun, $bulan, $tanggal);
         $getPartNameByShift = $this->getPartNameByShift($tahun, $bulan, $tanggal);
         $part_name = $this->getPartName($tahun, $bulan);
         $rejection_type = $this->getTypeName($tahun, $bulan);
         $data = array(
            ...
            'data_chart_1c' => $chart1cDataShift,
            'rejection_per_type_part_data' => $getRejectionPerTypePartData,
            'get_rejection_type_by_shift' => $getRejectionTypeByShift,
            'get_part_name_by_shift' => $getPartNameByShift,
            'part_name' => $part_name,
   ...
   public function getChart1aDataShift($tahun, $bulan, $tanggal)
   {
      $query = DB::select('
               select * from 
                  (
                     select 
                        count("RejectionType") as rejectbulantahun1, 
                        "RejectionType",
                        EXTRACT(DAY FROM "InputDate" + INTERVAL ' . "'1 hours'" . ') as tgl,
                        EXTRACT(MONTH FROM "InputDate" + INTERVAL ' . "'1 hours'" . ') as bulan,
                        EXTRACT(YEAR FROM "InputDate" + INTERVAL ' . "'1 hours'" . ') as tahun from "Pi_Rejection" where
                        ...
                        EXTRACT(MONTH FROM "InputDate" + INTERVAL ' . "'1 hours'" . ') = ' . $bulan . ' and
                        EXTRACT(DAY FROM "InputDate" + INTERVAL ' . "'1 hours'" . ') = ' . $tanggal . '
                        group by "RejectionType",tgl, bulan, tahun 
                        order by rejectbulantahun1 
                        desc 
                        limit 10
                  ) as t1
		            left join 
                  (
                     select 
                        ...
                        EXTRACT(DAY FROM "InputDate" + INTERVAL ' . "'1 hours'" . ') = ' . $tanggal . '
                        group by "RejectionType",tgl, bulan, tahun
                  ) as t2 on "t1"."RejectionType" = "t2"."RejectionType" 
                  order by rejectbulantahun1 
                  desc'
               );
   ...
   public function getChart1bDataShift($tahun, $bulan, $tanggal)
   {
      ...
      foreach ($query as $data) {
         $rejection_data[$data->shift][$data->RejectionType] = [$data->jumlah_shift_duplikat];
      }

      return $rejection_data;
   }
   
   public function getChart1cDataShift($tahun, $bulan, $tanggal)
   {
      $query = DB::select('
                  SELECT "NamaPart", "RejectionType", "tgl", "bulan", "tahun", "shift", COUNT(*) jumlah_shift_duplikat FROM 
                  ( 
      ...
      foreach ($query as $data) {
         $rejection_data[$data->shift][$data->NamaPart] = [$data->jumlah_shift_duplikat];
      }

      return $rejection_data;
   }

   public function getPartNameByShift($tahun, $bulan, $tanggal)
   {
      $query = DB::select('
                  SELECT "NamaPart", "RejectionType", "tgl", "bulan", "tahun", "shift", COUNT(*) jumlah_shift_duplikat FROM 
                  ( 
   ...
   public function getRejectionTypeByShift($tahun, $bulan, $tanggal)
   {
      $query = DB::select('
                  SELECT "NamaPart", "RejectionType", "tgl", "bulan", "tahun", "shift", COUNT(*) jumlah_shift_duplikat FROM 
                  ( 
